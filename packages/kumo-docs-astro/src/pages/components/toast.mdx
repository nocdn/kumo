...
<p>
  Give repeated notifications a stable <code>id</code> to keep one toast on
  screen. Click quickly: each trigger refreshes and bumps the current toast
  instead of stacking another copy.
</p>
<ComponentExample demo="ToastDeduplicatedDemo">
  <ToastDeduplicatedDemo client:load />
</ComponentExample>

### Functional Updates

<p>
  Update a toast in place from its current state. This example changes the
  deployment toast to success after it is created.
</p>
